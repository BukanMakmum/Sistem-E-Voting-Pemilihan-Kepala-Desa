...
$cek = mysqli_query($config,"select * from admin where admin_password='$password_lama'");
	...
	if(strlen($password_baru) >= 8){
		...
		if($password_baru == $konfirmasi_password){
			...
			$password_baru 	= md5($password_baru);
					...
					$update = mysqli_query($config,"update admin set admin_password='$password_baru' where admin_id='$id'");
